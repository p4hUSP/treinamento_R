...
node %>% xml2::write_html("web_scraping/data_raw/estante_virtual.html")
...
## tipo do livro
tipo <- node %>%
  rvest::html_nodes(xpath = '//p[@class="busca-type type m-used"]') %>% 
  rvest::html_text(trim = T)
...
## frete
node %>% 
  rvest::html_nodes(xpath = '//div[@class="breslt-buttons-price-wrapper"]//span[@class="busca-label"]/text()') %>% rvest::html_text()
```
